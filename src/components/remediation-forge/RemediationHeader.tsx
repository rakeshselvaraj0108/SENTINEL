...
import { remediationCve } from "@/lib/remediation-data";
...
export function RemediationHeader({ state, onStateChange, verifyEnabled }: RemediationHeaderProps) {
  return (
    <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-border-soft px-4 py-3">
      <div className="flex items-baseline gap-3">
        <h1 className="text-[14px] font-medium text-text">
          Remediation Strategy — <span className="text-text-muted">SQLi /api/checkout</span>
        </h1>
      ...
      <div className="flex items-center border border-border-soft text-[11px] uppercase tracking-[0.06em]">
        <button
          ...
          className={clsx(
            ...
            state === "generate" ? "bg-amber-soft text-amber" : "text-text-muted hover:text-text"
          )}
        >
          Generate
        </button>
