...
import type { DiffLine } from "@/lib/remediation-types";
...
export function DiffCodeBlock({ lines, isDiff }: { lines: DiffLine[]; isDiff: boolean }) {
  return (
    <pre className="overflow-x-auto p-2 font-data text-[10.5px] leading-[1.6]">
      {lines.map((line, i) => (
        <div key={i} className={clsx("flex gap-2 whitespace-pre px-1", lineStyle[line.kind])}>
          {isDiff && <span className="w-3 shrink-0 select-none text-text-dim">{marker[line.kind]}</span>}
          <span>{line.text || " "}</span>
        </div>
      ))}
    </pre>
